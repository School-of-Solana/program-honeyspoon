...
export async function GET(request: NextRequest) {
  ...
  console.log('[SSE] New connection request:', { userId, hasHouseAuth: !!houseAuthority });

  // Create a ReadableStream for SSE
  const stream = new ReadableStream({
    start(controller) {
      connections.add(controller);
      console.log('[SSE] Connection established. Active connections:', connections.size);
      ...
      const sendUpdate = async () => {
        try {
          ...
          const updates: any = { type: 'balance-update', timestamp: Date.now() };
          ...
          if (houseAuthority) {
            try {
              ...
              if (useSolana) {
                ...
                const [vaultPdaPubkey] = getHouseVaultAddress(houseAuthPubkey, programId);
              ...
              if (vaultState) {
                const vaultBalanceLamports = await chain.getVaultBalance(vaultPda);
                updates.houseVaultBalance = lamportsToSol(vaultBalanceLamports);
                updates.houseVaultReserved = lamportsToSol(vaultState.totalReserved);
              }
            } catch (error) {
              console.error('[SSE] Failed to fetch house vault balance:', error);
            }
          }
      ...
      // Then send updates every 3 seconds (more frequent than polling)
