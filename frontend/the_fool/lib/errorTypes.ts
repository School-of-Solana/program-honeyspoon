...
export const ERROR_MESSAGES: Record<GameErrorCode, string> = {
  ...
  [GameErrorCode.SESSION_INVALID]:
    "Game session expired. Please start a new game.",
  [GameErrorCode.SESSION_INACTIVE]:
    "Game session is no longer active. Starting new game...",
  [GameErrorCode.SESSION_NOT_OWNED]:
    "Invalid session ownership. Please refresh.",
  [GameErrorCode.SESSION_EXPIRED]: "Session timed out. Starting new game...",

  // Validation errors
  [GameErrorCode.TREASURE_MISMATCH]:
    "Treasure amount mismatch detected. Please contact support.",
  [GameErrorCode.ROUND_MISMATCH]:
    "Round number mismatch. Please refresh the game.",
  [GameErrorCode.CASHOUT_MISMATCH]:
    "Cash-out amount mismatch. Please contact support.",
...
export function parseServerError(errorMessage: string): GameError {
  ...
  if (msg.includes("cash-out mismatch")) {
    const match = errorMessage.match(
      /Session has \$(\d+), attempting to cash out \$(\d+)/
    );
  ...
  // Unknown error
  return createGameError(GameErrorCode.UNKNOWN, {
