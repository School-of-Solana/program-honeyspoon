...
export class SessionCleanupManager {
  ...
  static async cleanupAbandonedSession(
    ...
    try {
      ...
      console.warn('[SessionCleanup] Active abandoned session found:', sessionPda);
      console.warn('[SessionCleanup] Manual cleanup required - call lose_session or wait for timeout');
  ...
  static async initialize(connection: Connection): Promise<void> {
    ...
    if (cleaned) {
      console.log('[SessionCleanup] ✅ Successfully cleaned up abandoned session');
    } else {
      console.warn('[SessionCleanup] ⚠️ Abandoned session still active - may need manual cleanup');
    }
  }
}
