  ...
  try {
    ...
    if (blockchainConfig) {
      ...
      cachedConfig = {
        ...
        BASE_SURVIVAL_PROBABILITY: (blockchainConfig.baseSurvivalPpm /
          1_000_000) as number,
        DECAY_CONSTANT: (blockchainConfig.decayPerDivePpm /
          1_000_000) as number,
        MIN_WIN_PROB: (blockchainConfig.minSurvivalPpm / 1_000_000) as number,
        TREASURE_MULTIPLIER: (blockchainConfig.treasureMultiplierNum /
          blockchainConfig.treasureMultiplierDen) as number,
        HOUSE_EDGE: (1 -
          blockchainConfig.treasureMultiplierNum /
            blockchainConfig.treasureMultiplierDen) as number,
    ...
    console.warn(
      "[GameConfig] Failed to load from blockchain, using fallback:",
      error
    );
  }
