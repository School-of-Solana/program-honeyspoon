...
export function WalletMultiButton() {
  ...
  const base = 'inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-medium transition-all';
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {dropdownOpen && (
          <div className="absolute right-0 mt-2 w-64 bg-gray-800 border-2 border-gray-700 rounded-lg shadow-xl z-50">
            ...
            {/* Change Wallet */}
            <div className="p-2">
              <div className="text-xs text-gray-400 px-2 py-1">Change Wallet</div>
              {wallets.filter((w) => w.readyState === 'Installed').map((w) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 hover:bg-gray-700 rounded transition-colors ${
                    w.adapter.name === wallet.adapter.name ? 'bg-gray-700' : ''
                  }`}
              ...
              {wallets.filter((w) => w.readyState === 'NotDetected').length > 0 && (
                <>
                  <div className="text-xs text-gray-500 px-2 py-1 mt-2">Not Installed</div>
                  {wallets.filter((w) => w.readyState === 'NotDetected').map((w) => (
                    <a
                      key={w.adapter.name}
                      href={w.adapter.url}
                      target="_blank"
                    ...
                    >
                      ...
                      />
                      <span className="text-sm text-gray-400">{w.adapter.name}</span>
