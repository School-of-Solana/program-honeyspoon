...
describe('Concurrent Bet Placement', () => {
  ...
  it('should handle multiple simultaneous bets from same user', async () => {
    ...
    console.log(`✓ ${successCount}/3 concurrent bets succeeded, balance: $${finalWallet.balance}`);
  });
...
describe('Concurrent Game Operations', () => {
  ...
  it('should handle concurrent dive and cash-out attempts', async () => {
    ...
    const dive2 = performDive(2, diveResult.totalTreasure, sessionId, userId, '99');
    const cashOut = surfaceWithTreasure(diveResult.totalTreasure, sessionId, userId);
    ...
    console.log(`✓ Concurrent dive/cash-out: ${successCount} succeeded (race handled)`);
  });
...
describe('House Wallet Race Conditions', () => {
  ...
  it('should handle multiple concurrent payouts', async () => {
    ...
    // Create 5 users and start games
    for (let i = 0; i < 5; i++) {
    ...
    console.log(`✓ 5 concurrent payouts: house $${houseBefore.balance} → $${houseAfter.balance}`);
  });
  ...
  it('should handle house running out of funds gracefully', async () => {
    ...
    assert.strictEqual(house.canAcceptBets, false, 'House should indicate it cannot accept bets');
    
    console.log(`✓ House limits: ${acceptedBets}/100 bets accepted before limit`);
  });
...
describe('Session State Corruption', () => {
  ...
  it('should handle session data consistency after multiple operations', async () => {
    ...
    for (let dive = 1; dive <= 5; dive++) {
      ...
      assert.strictEqual(session!.diveNumber, dive, `Session should expect dive ${dive}`);
      assert.strictEqual(session!.userId, userId, 'Session should belong to user');
    ...
    assert.strictEqual(finalSession!.diveNumber, 6, 'Session should expect dive 6');
...
describe('Balance Update Consistency', () => {
  ...
  it('should update balance atomically', async () => {
    ...
    assert.strictEqual(balanceAfter, balanceBefore - 50, 'Balance update should be atomic');
  ...
  it('should track total wagered correctly', async () => {
    ...
    console.log(`✓ Total wagered tracked: $${initialWallet.totalWagered} → $${finalWallet.totalWagered}`);
  });
  ...
  it('should track wins and losses separately', async () => {
    ...
    assert.ok(finalWallet.totalLost > initialWallet.totalLost, 'Should track losses');
    assert.ok(finalWallet.totalWon > initialWallet.totalWon, 'Should track wins');
    assert.strictEqual(finalWallet.gamesPlayed, initialWallet.gamesPlayed + 2, 'Should count both games');
    
    console.log(`✓ Win/loss tracking: ${finalWallet.totalWon} won, ${finalWallet.totalLost} lost`);
  });
