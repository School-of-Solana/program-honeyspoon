...
function createMockHouseWallet(balance: number = 50000, reservedFunds: number = 0): HouseWallet {
  return {
    balance,
    totalPaidOut: 0,
    totalReceived: 0,
...
describe('Wallet Logic - calculateMaxPotentialPayout', () => {
  ...
  it('should scale linearly with bet amount', () => {
    ...
    console.log(`✓ Payout scales linearly: $100→$${payout100}, $200→$${payout200} (ratio: ${ratio.toFixed(2)})`);
  });
...
describe('Wallet Logic - calculateMaxBetFromHouseWallet', () => {
  it('should calculate safe max bet for healthy house', () => {
    ...
    assert.ok(maxBet <= DEFAULT_LIMITS.maxBet, 'Should not exceed absolute max');
  ...
  it('should reduce max bet when house has reserved funds', () => {
    ...
    assert.ok(maxBet2 <= maxBet1, 'Max bet should be lower or equal with reserved funds');
...
describe('Wallet Logic - validateBet', () => {
  ...
  it('should reject bet below minimum', () => {
    ...
    assert.strictEqual(validation.valid, false, 'Should reject bet below minimum');
    assert.ok(validation.error?.includes('Minimum bet'), 'Error should mention minimum bet');
  ...
  it('should reject bet exceeding user balance', () => {
    ...
    assert.strictEqual(validation.valid, false, 'Should reject bet exceeding balance');
    assert.ok(validation.error?.includes('Insufficient balance'), 'Error should mention insufficient balance');
    assert.strictEqual(validation.userBalance, 50, 'Should return user balance');
  ...
  it('should reject bet when house cannot cover', () => {
    ...
    if (!validation.valid) {
      assert.ok(
        validation.error?.includes('House cannot') || validation.error?.includes('Maximum bet'),
  ...
  it('should reject bet above absolute maximum', () => {
    ...
    assert.ok(validation.error?.includes('Maximum bet'), 'Error should mention maximum bet');
...
describe('Wallet Logic - validateDiveDeeper', () => {
  ...
  it('should block dive when house cannot afford increase', () => {
    ...
    if (!validation.valid) {
      assert.ok(validation.error?.includes('House cannot'), 'Error should mention house capacity');
  ...
  it('should block dive when approaching max win', () => {
    ...
    if (!validation.valid) {
      assert.ok(
        validation.error?.includes('maximum') || validation.error?.includes('surface'),
...
describe('Wallet Logic - User Wallet Transactions', () => {
  it('should process bet correctly', () => {
    ...
    assert.strictEqual(updated.totalWagered, 100, 'Total wagered should be $100');
  ...
  it('should process win correctly', () => {
    ...
    assert.strictEqual(updated.totalWon, 400, 'Profit should be $400 ($500 - $100)');
  ...
  it('should process loss correctly', () => {
    ...
    assert.strictEqual(updated.balance, 900, 'Balance should not change (already deducted)');
...
describe('Wallet Logic - House Wallet Transactions', () => {
  it('should receive bet correctly', () => {
    ...
    assert.strictEqual(updated.totalReceived, 100, 'Total received should be $100');
  ...
  it('should process payout correctly', () => {
    ...
    assert.strictEqual(updated.totalPaidOut, 500, 'Total paid out should be $500');
  ...
  it('should handle partial reserve release', () => {
    ...
    assert.strictEqual(updated.reservedFunds, 5000, 'Should release only $5k of $10k');
...
describe('Wallet Logic - Risk Exposure', () => {
  it('should calculate risk exposure correctly', () => {
    ...
    assert.strictEqual(exposure.totalReserved, 10000, 'Should show $10k reserved');
    
    // Available = balance - reserved - (20% reserve)
    // = 50000 - 10000 - 10000 = 30000
    assert.strictEqual(exposure.availableFunds, 30000, 'Should have $30k available');
    assert.strictEqual(exposure.reserveRequired, 10000, 'Should require $10k reserve (20%)');
    
    console.log(`✓ Risk exposure: $${exposure.availableFunds} available of $${house.balance}`);
  });
  ...
  it('should indicate when house can accept bets', () => {
    ...
    assert.strictEqual(exposure.canAcceptNewBets, true, 'Should accept new bets');
  ...
  it('should calculate max new bet correctly', () => {
    ...
    assert.ok(exposure.maxNewBet <= DEFAULT_LIMITS.maxBet, 'Should not exceed absolute max');
  ...
  it('should handle edge case: fully reserved house', () => {
    ...
    assert.strictEqual(exposure.availableFunds, 0, 'Should have no available funds');
    assert.strictEqual(exposure.canAcceptNewBets, false, 'Should not accept bets');
...
describe('Wallet Logic - Edge Cases', () => {
  ...
  it('should handle user with zero balance', () => {
    ...
    assert.strictEqual(validation.valid, false, 'Should reject when balance is zero');
  ...
  it('should handle house with zero balance', () => {
    ...
    assert.strictEqual(validation.valid, false, 'Should reject when house is empty');
  ...
  it('should handle very large bet amounts', () => {
    ...
    assert.strictEqual(validation.valid, false, 'Should reject extremely large bets');
