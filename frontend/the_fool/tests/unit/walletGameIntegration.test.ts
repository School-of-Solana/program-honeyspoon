...
describe('Wallet + Game Integration - Full Game Flow', () => {
  ...
  it('should complete successful game with win', () => {
    ...
    assert.strictEqual(userAfterBet.balance, 1000 - initialBet, `User balance should be $${1000 - initialBet}`);
    assert.strictEqual(house.balance, 50000 + initialBet, `House balance should be $${50000 + initialBet}`);
    assert.strictEqual(house.reservedFunds, maxPayout, 'House should reserve max payout');
    ...
    assert.strictEqual(userAfterWin.balance, (1000 - initialBet) + currentTreasure, 'User should receive treasure');
    assert.strictEqual(house.balance, (50000 + initialBet) - currentTreasure, 'House should pay out');
    assert.strictEqual(house.reservedFunds, 0, 'Reserves should be released');
    
    const profit = currentTreasure - initialBet;
    assert.strictEqual(userAfterWin.totalWon, profit, 'Profit should be tracked');
    assert.strictEqual(userAfterWin.gamesPlayed, 1, 'Games played should increment');
    
    console.log(`✓ Game won: $${initialBet} → $${currentTreasure} (profit: $${profit})`);
  });
  ...
  it('should complete game with loss', () => {
    ...
    assert.strictEqual(userAfterLoss.balance, 900, 'User balance should stay $900 (already deducted)');
    assert.strictEqual(userAfterLoss.totalLost, initialBet, 'Loss should be tracked');
    assert.strictEqual(userAfterLoss.gamesPlayed, 1, 'Games played should increment');
    assert.strictEqual(house.balance, 50100, 'House keeps the bet');
    assert.strictEqual(house.reservedFunds, 0, 'Reserves should be released');
    
    console.log(`✓ Game lost: $${initialBet} bet lost, balance: $${userAfterLoss.balance}`);
  });
  ...
  it('should handle multiple consecutive games', () => {
    ...
    assert.strictEqual(house.reservedFunds, 0, 'All reserves should be released');
  ...
  it('should manage game session lifecycle', () => {
    ...
    assert.strictEqual(retrieved?.currentTreasure, 300, 'Treasure should update');
  ...
  it('should prevent betting when house cannot cover', () => {
    ...
    assert.ok(validation.error?.includes('House') || validation.error?.includes('Maximum'), 'Error should mention limit');
  ...
  it('should handle concurrent games with reserved funds', () => {
    ...
    if (validation.valid) {
      ...
      assert.strictEqual(house.reservedFunds, maxPayout1 + maxPayout2, 'Reserves should accumulate');
  ...
  it('should calculate correct profit/loss over time', () => {
    ...
    assert.strictEqual(user.balance, expectedBalance, 'Balance should match net profit');
    assert.strictEqual(user.gamesPlayed, 5, 'Should track 5 games');
    
    console.log(`✓ After 5 games: balance=$${user.balance}, won=$${user.totalWon}, lost=$${user.totalLost}`);
  });
});
...
describe('Wallet + Game Integration - Edge Cases', () => {
  ...
  it('should handle user running out of money', () => {
    ...
    assert.ok(validation.error?.includes('Insufficient'), 'Error should mention balance');
  ...
  it('should maintain consistency after many operations', () => {
    ...
    assert.strictEqual(house.balance, expectedHouseBalance, 'House balance should match');
    assert.strictEqual(house.reservedFunds, 0, 'No funds should be reserved');
    
    console.log(`✓ 50 games: house profit=$${houseProfit}, balance=$${house.balance}`);
  });
  ...
  it('should handle extreme payout scenarios', () => {
    ...
    assert.ok(maxPayout >= treasure, 'Max payout calculation should cover actual payout');
    ...
    console.log(`✓ Extreme win: $${bet} → $${treasure}, house balance=$${house.balance}`);
  });
  ...
  it('should demonstrate house edge over many games', () => {
    ...
    assert.ok(actualPayout > expectedAverage, 'Lucky player beats expected value');
    
    // But on average, players lose 15% per dive
    assert.ok(cumulativeEV < 1, 'Expected value decreases (house edge)');
    
    console.log(`✓ House edge: Lucky player wins $${actualPayout}, but average would be $${expectedAverage.toFixed(2)} (${(cumulativeEV * 100).toFixed(1)}% of bet)`);
  });
});
