...
describe('Probability Verification', () => {
  ...
  it('should verify Round 1 has 95% survival rate (statistical test)', async () => {
    ...
    const tolerance = 0.10; // ±10% due to small sample
    
    console.log(`  Survival rate: ${(survivalRate * 100).toFixed(1)}% (expected: ${(expectedRate * 100).toFixed(1)}%)`);
    console.log(`  Survived: ${survived}/${trials}`);
    
    assert.ok(
      survivalRate >= expectedRate - tolerance && survivalRate <= expectedRate + tolerance,
    ...
    for (const round of [1, 5, 10]) {
      ...
      for (let i = 0; i < trialsPerRound; i++) {
        ...
        while (currentRound <= round) {
          const result = await performDive(currentRound, treasure, sessionId, userId);
      ...
      console.log(`  Round ${round}: ${(survivalRates[round] * 100).toFixed(1)}% survival (${survived}/${trialsPerRound})`);
    }
  ...
  it('should verify house edge over many games', async () => {
    ...
    console.log(`  Actual edge: ${(actualEdge * 100).toFixed(1)}% (expected: 15%)`);
...
describe('Randomness Tests', () => {
  ...
  it('should have unpredictable random rolls', async () => {
    ...
    assert.ok(avg >= 40 && avg <= 60, `Average roll should be ~50, got ${avg.toFixed(1)}`);
    ...
    const variance = rolls.reduce((sum, r) => sum + Math.pow(r - avg, 2), 0) / rolls.length;
    const stdDev = Math.sqrt(variance);
    assert.ok(stdDev >= 20 && stdDev <= 35, `StdDev should be ~28.87, got ${stdDev.toFixed(1)}`);
  ...
  it('should have uniform distribution of rolls', async () => {
    ...
    console.log(`  Chi-square: ${chiSquare.toFixed(2)} (lower is more uniform)`);
...
describe('Multiplier Calculations', () => {
  ...
  it('should verify treasure decreases on average', async () => {
    ...
    console.log(`  Average treasure after dive 1: $${avgTreasure.toFixed(2)} (started with $50)`);
    
    // On average, should be less than $50 due to house edge
    assert.ok(avgTreasure < 50, 'Average treasure should decrease due to house edge');
    
    // Should be around $42-$45 (50 * 0.85)
    assert.ok(avgTreasure >= 35 && avgTreasure <= 48, 'Average should be ~$42.50');
  ...
  it('should handle very small treasure values', async () => {
    ...
    for (let dive = 1; dive <= 10; dive++) {
      ...
      assert.ok(treasure >= 0, `Treasure should be non-negative, got ${treasure}`);
  ...
  it('should handle very large treasure values', async () => {
    ...
    for (let dive = 1; dive <= 20; dive++) {
      ...
      // Verify no overflow
      assert.ok(treasure < Number.MAX_SAFE_INTEGER, 'Treasure should not overflow');
...
describe('Floating Point Precision', () => {
  it('should handle floating point rounding in multipliers', async () => {
    ...
    for (let dive = 1; dive <= 5; dive++) {
      ...
      assert.ok(Number.isInteger(result.totalTreasure), 'Treasure should be integer');
