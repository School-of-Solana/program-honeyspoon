...
describe('Treasure Initialization - Bug Fixes', () => {
  ...
  it('should start with treasure = 0 (not betAmount)', async () => {
    ...
    console.log(`✓ Game started: bet=$${BET_AMOUNT}, balance=$${walletAfterBet.balance}`);
  ...
  it('should calculate first dive treasure correctly (betAmount * multiplier)', async () => {
    ...
    const diveResult = await performDive(1, BET_AMOUNT, sessionId, userId, '99');
    ...
    console.log(`✓ First dive: $${BET_AMOUNT} * ${diveResult.multiplier.toFixed(2)}x = $${diveResult.totalTreasure}`);
  ...
  it('should accumulate treasure correctly across multiple dives', async () => {
    ...
    // First dive
    const dive1 = await performDive(1, BET_AMOUNT, newSessionId, newUserId, '99');
    assert.strictEqual(dive1.survived, true, 'Dive 1 should survive');
    
    const treasure1 = dive1.totalTreasure;
    console.log(`  Dive 1: $${BET_AMOUNT} → $${treasure1} (${dive1.multiplier.toFixed(2)}x)`);

    // Second dive
    const dive2 = await performDive(2, treasure1, newSessionId, newUserId, '99');
    ...
    console.log(`  Dive 2: $${treasure1} → $${treasure2} (${dive2.multiplier.toFixed(2)}x)`);

    // Third dive
    const dive3 = await performDive(3, treasure2, newSessionId, newUserId, '99');
    ...
    console.log(`  Dive 3: $${treasure2} → $${treasure3} (${dive3.multiplier.toFixed(2)}x)`);
    ...
    console.log(`✓ Treasure accumulation: $${BET_AMOUNT} → $${treasure1} → $${treasure2} → $${treasure3}`);
  });
  ...
  it('should update wallet balance correctly after surfacing', async () => {
    ...
    // Surface
    const surfaceResult = await surfaceWithTreasure(finalTreasure, sessionId, userId);
    ...
    console.log(`✓ Wallet updates: $${initialBalance} → $${walletInfo.balance}`);
  ...
  it('should handle the exact scenario from bug report', async () => {
    ...
    console.log(`  ✓ Step 2: First dive successful → Treasure = $${treasure1} (not 50!)`);
    ...
    console.log(`  ✓ Step 3: Second dive successful → Treasure = $${treasure2}`);
    console.log(`    (multiplied from $${treasure1}, not added!)`);

    // Step 4: Surface
    const surfaceResult = await surfaceWithTreasure(treasure2, newSessionId, newUserId);
    assert.strictEqual(surfaceResult.success, true, 'Surface should succeed');
    ...
    console.log(`    Net result: ${wallet.balance - 1000 > 0 ? '+' : ''}$${wallet.balance - 1000}`);
  ...
  it('should verify treasure never starts at betAmount (regression test)', async () => {
    ...
    console.log(`✓ Regression test passed: treasure = $${dive1.totalTreasure} (not $${BET_AMOUNT})`);
  });
});
...
describe('Edge Cases - Zero Treasure Handling', () => {
  ...
  it('should handle currentTreasure = 0 by using betAmount on first dive', async () => {
    ...
    assert.ok(dive1.totalTreasure < 50, 'Treasure should be less than bet (house edge)');
    
    console.log(`✓ First dive with betAmount=50 → treasure=$${dive1.totalTreasure}`);
  });
  ...
  it('should fail if we accidentally pass 0 on first dive', async () => {
    ...
    assert.strictEqual(dive1.totalTreasure, 0, 'BUG: Passing 0 results in 0 treasure!');
    
    console.log(`✗ BUG DEMO: First dive with value=0 → treasure=$${dive1.totalTreasure} (broken!)`);
