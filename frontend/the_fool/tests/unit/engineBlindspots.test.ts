...
describe('Bet Validation - Boundary Values', () => {
  ...
  it('should reject just below minBet', () => {
    ...
    assert.strictEqual(result.valid, false, 'Should reject 9.99 (below minBet)');
  ...
  it('should reject just above maxBet', () => {
    ...
    assert.strictEqual(result.valid, false, 'Should reject 500.01 (above maxBet)');
  ...
  it('should handle non-integer valid bets', () => {
    const result = validateBetAmount(15.50, DEFAULT_CONFIG);
...
describe('Round Stats - Edge Cases', () => {
  it('should handle round 1 with exact baseline probability', () => {
    ...
    assert.ok(Math.abs(stats.winProbability - 0.95) < 0.01, 'Round 1 should have ~95% probability');
    ...
    console.log(`  Round 1: P=${stats.winProbability}, M=${stats.multiplier.toFixed(3)}x, EV=${ev.toFixed(3)}`);
  });
  ...
  it('should handle very large round numbers at maxRounds', () => {
    ...
    assert.ok(stats.winProbability >= DEFAULT_CONFIG.minWinProbability, 'Should not go below min');
    assert.ok(Number.isFinite(stats.multiplier), 'Multiplier should be finite');
    
    console.log(`  Round 50: P=${(stats.winProbability * 100).toFixed(2)}%, M=${stats.multiplier.toFixed(2)}x`);
  });
  ...
  it('should reject round 0', () => {
    try {
      ...
      assert.ok((error as Error).message.includes('must be >= 1'), 'Should mention minimum');
    }
  });
  ...
  it('should reject round beyond maxRounds', () => {
    try {
      ...
      assert.ok((error as Error).message.includes('exceeds maximum'), 'Should mention maximum');
    }
  });
...
describe('Invalid Config Tests', () => {
  ...
  it('should handle houseEdge = 1 (100% edge)', () => {
    ...
    assert.strictEqual(stats.multiplier, 0, 'Multiplier should be 0 with 100% house edge');
  });
  ...
  it('should handle very low baseWinProbability', () => {
    ...
    assert.strictEqual(stats.winProbability, 0.1, 'Should use low base probability');
    assert.ok(stats.multiplier > 5, 'Multiplier should be high with low probability');
  });
  ...
  it('should handle baseWinProbability = 1', () => {
    ...
    assert.strictEqual(stats.winProbability, 1.0, 'Should have 100% win probability');
    assert.strictEqual(stats.multiplier, 0.85, 'Multiplier should be 0.85 (1 - 0.15 edge)');
  });
...
describe('Max Potential Payout', () => {
  it('should respect maxPotentialWin cap', () => {
    ...
    const maxPayout = calculateMaxPotentialPayout(bet, maxRounds, DEFAULT_CONFIG);
    ...
    console.log(`  Max payout for $${bet}: $${maxPayout} (capped at ${DEFAULT_CONFIG.maxPotentialWin})`);
  });
  ...
  it('should handle maxRounds = 1', () => {
    ...
    assert.strictEqual(payout, expected, `Should match round 1 multiplier: ${expected}`);
  });
  ...
  it('should increase with more rounds', () => {
    ...
    assert.ok(payout5 > payout1, '5 rounds should have higher max than 1 round');
    assert.ok(payout10 > payout5, '10 rounds should have higher max than 5 rounds');
    
    console.log(`  Max payout progression: 1R=$${payout1}, 5R=$${payout5}, 10R=$${payout10}`);
  });
...
describe('Simulate Round - Boundary Rolls', () => {
  it('should handle roll = 0 (lowest possible)', () => {
    ...
    assert.strictEqual(result.survived, shouldSurvive, `Roll 0 vs threshold ${stats.threshold}`);
  ...
  it('should handle roll = 99 (highest possible)', () => {
    ...
    assert.strictEqual(result.survived, true, 'Roll 99 should always survive round 1');
  ...
  it('should handle roll exactly at threshold', () => {
    ...
    assert.strictEqual(result.survived, true, 'Roll at threshold should survive');
  });
  ...
  it('should handle roll just below threshold', () => {
    ...
    assert.strictEqual(result.survived, false, 'Roll below threshold should fail');
  });
  ...
  it('should reject invalid roll < 0', () => {
    try {
      ...
      assert.ok((error as Error).message.includes('must be 0-99'), 'Should mention valid range');
    }
  });
  ...
  it('should reject negative currentValue', () => {
    try {
      ...
      assert.ok((error as Error).message.includes('non-negative'), 'Should mention non-negative');
    }
  });
