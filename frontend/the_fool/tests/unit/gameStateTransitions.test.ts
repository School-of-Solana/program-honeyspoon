...
describe('Game State Transitions', () => {
  ...
  it('should transition from idle → betting → playing', async () => {
    ...
    if (diveResult.survived) {
      ...
      assert.strictEqual(updatedSession!.diveNumber, 2, 'Should advance to dive 2');
      assert.strictEqual(updatedSession!.isActive, true, 'Should still be active');
    }
  ...
  it('should transition from playing → game over (loss)', async () => {
    ...
    if (!diveResult.survived) {
      ...
      const session = getGameSession(sessionId);
      assert.strictEqual(session, undefined, 'Session should be deleted after loss');
  ...
  it('should transition from playing → surfacing → game over (win)', async () => {
    ...
    assert.strictEqual(surfaceResult.success, true, 'Should surface successfully');
    
    // Should transition to game over
    const session = getGameSession(sessionId);
    assert.strictEqual(session, undefined, 'Session should be deleted after surface');
  ...
  it('should handle multiple dives (stay in playing state)', async () => {
    ...
    for (let dive = 1; dive <= 5; dive++) {
      ...
      assert.strictEqual(session!.isActive, true, 'Session should still be active');
      assert.strictEqual(session!.diveNumber, dive + 1, `Should be at dive ${dive + 1}`);
  ...
  it('should handle rapid state transitions (stress test)', async () => {
    ...
    for (let game = 1; game <= 10; game++) {
      ...
      // Dive
      const diveResult = await performDive(1, 10, newSessionId, userId, `${game * 10}`);
      
      if (diveResult.survived) {
        // Surface
        await surfaceWithTreasure(diveResult.totalTreasure, newSessionId, userId);
      }
      
      // Verify session cleaned up
      const session = getGameSession(newSessionId);
      assert.strictEqual(session, undefined, `Session ${game} should be cleaned up`);
    }
...
describe('Session Management', () => {
  ...
  it('should clean up session after loss', async () => {
    ...
    assert.strictEqual(sessionAfter, undefined, 'Session should be deleted after loss');
  ...
  it('should clean up session after win', async () => {
    ...
    const session = getGameSession(sessionId);
    assert.strictEqual(session, undefined, 'Session should be deleted after win');
...
describe('Concurrent Action Prevention', () => {
  ...
  it('should handle dive during surface', async () => {
    ...
    const surface = surfaceWithTreasure(diveResult.totalTreasure, sessionId, userId);
    ...
    assert.strictEqual(surfaceResult.status, 'fulfilled', 'Surface should succeed');
