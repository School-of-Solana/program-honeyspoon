...
describe('Money Conservation - Single Game', () => {
  ...
  it('should conserve money when player loses', async () => {
    ...
    if (!diveResult.survived) {
      ...
      assert.strictEqual(totalAfter, totalBefore, 'Total money must be conserved on loss');
      
      // User lost bet
      assert.strictEqual(finalUser.balance, initialUser.balance - 50, 'User should lose bet');
      
      // House gained bet
      assert.strictEqual(finalHouse.balance, initialHouse.balance + 50, 'House should gain bet');
      
      console.log(`✓ Money conserved on loss: $${totalBefore} → $${totalAfter}`);
    } else {
  ...
  it('should conserve money when player wins', async () => {
    ...
    // Total money should be conserved
    assert.strictEqual(totalAfter, totalBefore, 'Total money must be conserved on win');
    ...
    assert.strictEqual(userChange + houseChange, 0, 'Money transfer should sum to zero');
    
    console.log(`✓ Money conserved on win: $${totalBefore} → $${totalAfter}`);
    console.log(`  User: ${userChange > 0 ? '+' : ''}$${userChange}, House: ${houseChange > 0 ? '+' : ''}$${houseChange}`);
  });
  ...
  it('should conserve money with multiple dives', async () => {
    ...
    assert.strictEqual(totalAfter, totalBefore, 'Money conserved after multiple dives');
    
    console.log(`✓ Money conserved with 3 dives (survived=${survived}): $${totalBefore} → $${totalAfter}`);
  });
...
describe('Money Conservation - Multiple Games', () => {
  ...
  it('should conserve money across 10 games', async () => {
    ...
    for (let game = 1; game <= 10; game++) {
      ...
      const result = await performDive(1, 10, sessionId, userId, `${game * 10}`);
    ...
    assert.strictEqual(totalAfter, totalBefore, 'Total money must be conserved across games');
    
    console.log(`✓ Money conserved across 10 games: $${totalBefore} → $${totalAfter}`);
  });
  ...
  it('should conserve money with concurrent users', async () => {
    ...
    const totalBefore = wallet1Before.balance + wallet2Before.balance + wallet3Before.balance + houseBefore.balance;
    ...
    const totalAfter = wallet1After.balance + wallet2After.balance + wallet3After.balance + houseAfter.balance;
    
    assert.strictEqual(totalAfter, totalBefore, 'Money conserved with concurrent users');
    
    console.log(`✓ Money conserved with 3 concurrent users: $${totalBefore} → $${totalAfter}`);
  });
...
describe('House Fund Reservation', () => {
  ...
  it('should reserve funds on game start', async () => {
    ...
    console.log(`✓ Reserved funds: $${reservedBefore} → $${reservedAfter} (+$${reservedAfter - reservedBefore})`);
  });
  ...
  it('should release funds on loss', async () => {
    ...
    if (!result.survived) {
      ...
      assert.ok(reservedAfter < reservedMid, 'Reserved funds should be released');
      
      console.log(`✓ Reserved funds released: $${reservedMid} → $${reservedAfter}`);
    } else {
      console.log('⚠ Survived with roll=0, skipping');
    }
  });
  ...
  it('should release funds on win', async () => {
    ...
    assert.ok(reservedAfter < reservedMid, 'Reserved funds should be released on win');
    
    console.log(`✓ Reserved funds released on win: $${reservedMid} → $${reservedAfter}`);
  });
  ...
  it('should not leak reserved funds across multiple games', async () => {
    ...
    console.log(`✓ No reserve leaks: $${houseBefore.reservedFunds} → $${houseAfter.reservedFunds}`);
  });
...
describe('Treasure Accumulation Correctness', () => {
  ...
  it('should never increase treasure on loss', async () => {
    ...
    if (!result.survived) {
      assert.strictEqual(result.totalTreasure, 0, 'Treasure should be 0 on loss');
  ...
  it('should handle large treasure values without overflow', async () => {
    ...
    for (let dive = 1; dive <= 15; dive++) {
      try {
        ...
        if (!result.survived) {
          console.log(`  Died at dive ${dive} (roll=99 but probability too low)`);
          break;
        }
        ...
        assert.ok(treasure <= Number.MAX_SAFE_INTEGER, 'Treasure should not exceed MAX_SAFE_INTEGER');
      } catch (error) {
    ...
    console.log(`  Max treasure reached: $${maxTreasure} after ${diveCount} dives`);
