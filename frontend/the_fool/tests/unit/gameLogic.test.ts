...
describe('Game Logic - calculateDiveStats', () => {
  it('should calculate stats for dive 1', () => {
    ...
    assert.ok(stats.survivalProbability > 0 && stats.survivalProbability <= 1, 'Survival prob should be 0-1');
    ...
    console.log(`✓ Dive 1: ${(stats.survivalProbability * 100).toFixed(1)}% survival, ${stats.multiplier}x multiplier`);
  });
  ...
  it('should calculate stats for dive 5', () => {
    ...
    console.log(`✓ Dive 5: ${(stats.survivalProbability * 100).toFixed(1)}% survival, ${stats.multiplier}x multiplier`);
  });
  ...
  it('should calculate stats for dive 10', () => {
    ...
    console.log(`✓ Dive 10: ${(stats.survivalProbability * 100).toFixed(1)}% survival, ${stats.multiplier}x multiplier`);
  });
  ...
  it('should decrease survival probability over dives', () => {
    ...
    assert.ok(dive1.survivalProbability > dive5.survivalProbability, 'Dive 1 should be easier than dive 5');
    assert.ok(dive5.survivalProbability > dive10.survivalProbability, 'Dive 5 should be easier than dive 10');
    
    console.log(`✓ Survival decreases: ${(dive1.survivalProbability * 100).toFixed(1)}% → ${(dive5.survivalProbability * 100).toFixed(1)}% → ${(dive10.survivalProbability * 100).toFixed(1)}%`);
  });
  ...
  it('should increase multiplier over dives', () => {
    ...
    assert.ok(dive1.multiplier < dive5.multiplier, 'Dive 5 multiplier should be higher than dive 1');
    assert.ok(dive5.multiplier < dive10.multiplier, 'Dive 10 multiplier should be higher than dive 5');
    
    console.log(`✓ Multiplier increases: ${dive1.multiplier}x → ${dive5.multiplier}x → ${dive10.multiplier}x`);
  });

  it('should maintain 0.85 EV across all dives', () => {
    for (let i = 1; i <= 20; i++) {
      const stats = calculateDiveStats(i);
      assert.strictEqual(stats.expectedValue, 0.85, `EV should be 0.85 for dive ${i}`);
    }
  ...
  it('should calculate mathematically correct EV', () => {
    for (let i = 1; i <= 10; i++) {
      ...
      assert.ok(Math.abs(calculatedEV - 0.85) < 0.01, `EV calculation incorrect for dive ${i}: ${calculatedEV}`);
    }
    
    console.log('✓ EV calculation mathematically verified (survivalProb × multiplier = 0.85)');
  });
  ...
  it('should calculate correct threshold for random rolls', () => {
    ...
    assert.strictEqual(stats.threshold, expectedThreshold, 'Threshold calculation incorrect');
    
    console.log(`✓ Threshold: ${stats.threshold} (roll must be >= ${stats.threshold} to survive)`);
  });
  ...
  it('should deplete oxygen over dives', () => {
    ...
    assert.strictEqual(dive20.oxygenRemaining, 20, 'Dive 20 oxygen should be 20');
  ...
  it('should respect minimum survival probability', () => {
    ...
    assert.ok(dive50.survivalProbability >= GAME_CONFIG.MIN_WIN_PROB, 'Should respect min win prob');
    assert.ok(dive100.survivalProbability >= GAME_CONFIG.MIN_WIN_PROB, 'Should respect min win prob');
    
    console.log(`✓ Survival probability floors at ${GAME_CONFIG.MIN_WIN_PROB} (${(GAME_CONFIG.MIN_WIN_PROB * 100).toFixed(1)}%)`);
  });
...
describe('Game Logic - getDepthZone', () => {
  ...
  it('should assign zones correctly across all depths', () => {
    ...
    for (const depth of depths) {
      ...
      assert.ok(depth <= zone.max, `${depth}m should be <= zone max ${zone.max}m`);
    }
  ...
  it('should include zone properties', () => {
    ...
    console.log(`✓ Zone properties: color=${zone.color}, light=${zone.light}, max=${zone.max}m`);
  });
});
...
describe('Game Logic - generateShipwreck', () => {
  it('should generate deterministic shipwreck', () => {
    ...
    assert.deepStrictEqual(wreck1, wreck2, 'Same seed should generate identical shipwreck');
  ...
  it('should generate different shipwrecks for different dives', () => {
    ...
    assert.notStrictEqual(wreck1.name, wreck2.name, 'Different dives should generate different wrecks');
  ...
  it('should generate different shipwrecks for different seeds', () => {
    ...
    assert.ok(different, 'Different seeds should likely generate different wrecks');
  ...
  it('should include all required shipwreck properties', () => {
    ...
    assert.strictEqual(typeof wreck.treasureValue, 'number', 'Should have treasure value');
  ...
  it('should increase treasure value with depth', () => {
    ...
    assert.ok(wreck5.treasureValue > wreck1.treasureValue, 'Dive 5 treasure should be more valuable');
    assert.ok(wreck10.treasureValue > wreck5.treasureValue, 'Dive 10 treasure should be more valuable');
    
    console.log(`✓ Treasure value scales: $${wreck1.treasureValue} → $${wreck5.treasureValue} → $${wreck10.treasureValue}`);
  });
  ...
  it('should format ID correctly', () => {
    ...
    assert.strictEqual(wreck.id, 'session123-5', 'ID should be seed-diveNumber');
  ...
  it('should generate valid ship names', () => {
    for (let i = 1; i <= 10; i++) {
      ...
      assert.ok(parts.length >= 2, `Name should have multiple parts: "${wreck.name}"`);
    }
  ...
  it('should handle extreme dive numbers', () => {
    ...
    assert.ok(wreck100.treasureValue > wreck1.treasureValue, 'Treasure should scale');
    
    console.log(`✓ Dive 100: depth=${wreck100.depth}m, treasure=$${wreck100.treasureValue}`);
  });
});
...
describe('Game Logic - getTreasureVisual', () => {
  it('should return small treasure for low values', () => {
    ...
    console.log(`✓ $50 treasure: size=${visual.size}, particles=${visual.particles}`);
  });
  ...
  it('should return medium treasure for moderate values', () => {
    ...
    console.log(`✓ $300 treasure: size=${visual.size}, particles=${visual.particles}`);
  });
  ...
  it('should return large treasure for high values', () => {
    ...
    console.log(`✓ $750 treasure: size=${visual.size}, particles=${visual.particles}`);
  });
  ...
  it('should return epic treasure for very high values', () => {
    ...
    console.log(`✓ $2000 treasure: size=${visual.size}, particles=${visual.particles}`);
  });
  ...
  it('should include all visual properties', () => {
    ...
    console.log(`✓ Visual properties: size=${visual.size}, glow=${visual.glow}, color=${visual.color}`);
  });
});
...
describe('Game Logic - validateBet', () => {
  ...
  it('should reject bet below minimum', () => {
    ...
    assert.ok(result.error?.includes('Minimum'), 'Error should mention minimum');
  ...
  it('should reject bet above maximum', () => {
    ...
    assert.ok(result.error?.includes('Maximum'), 'Error should mention maximum');
  ...
  it('should accept minimum bet', () => {
    ...
    assert.strictEqual(result.valid, true, `Should accept minimum bet $${GAME_CONFIG.MIN_BET}`);
  ...
  it('should accept maximum bet', () => {
    ...
    assert.strictEqual(result.valid, true, `Should accept maximum bet $${GAME_CONFIG.MAX_BET}`);
...
describe('Game Logic - calculateCumulativeEV', () => {
  ...
  it('should calculate EV for 2 dives', () => {
    ...
    assert.strictEqual(Math.round(ev * 10000) / 10000, 0.7225, 'EV after 2 dives should be 0.7225');
  ...
  it('should decrease exponentially with more dives', () => {
    ...
    console.log(`✓ EV decreases exponentially: ${ev1.toFixed(4)} → ${ev5.toFixed(4)} → ${ev10.toFixed(4)}`);
  });
  ...
  it('should calculate profit expectation', () => {
    ...
    assert.ok(expected < bet, 'Expected value should be less than bet (house edge)');
    
    console.log(`✓ Bet $${bet}, 5 dives → expected value: $${expected.toFixed(2)} (${((1 - ev5) * 100).toFixed(1)}% house edge)`);
  });
});
...
describe('Game Logic - Edge Cases & Integration', () => {
  it('should handle extreme dive numbers gracefully', () => {
    ...
    console.log(`✓ Dive 1000: ${(dive1000.survivalProbability * 100).toFixed(1)}% survival, ${dive1000.multiplier}x multiplier`);
  });
  ...
  it('should maintain consistency between related functions', () => {
    ...
    assert.strictEqual(dive5.depth, wreck.depth, 'Dive and wreck depth should match');
    assert.strictEqual(zone.name, dive5.depthZone.name, 'Zone names should match');
  ...
  it('should generate unique shipwrecks for long sessions', () => {
    ...
    assert.ok(wrecks.size >= 40, `Should have mostly unique wrecks, got ${wrecks.size}/50`);
  ...
  it('should validate bet amounts at boundaries', () => {
    ...
    for (const test of tests) {
      ...
      assert.strictEqual(result.valid, test.valid, `Bet $${test.amount} validation incorrect`);
    }
  ...
  it('should calculate payout correctly for multi-dive scenario', () => {
    ...
    assert.ok(treasure > 100, 'Treasure should increase after successful dives');
