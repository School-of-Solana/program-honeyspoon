...
describe("Server Actions - Basic Operations", () => {
  ...
  it("should generate unique session IDs", async () => {
    ...
    assert.ok(
      id1.length >= 32,
      "ID should be at least 32 chars (16 bytes hex)"
    );
  ...
  it("should get house status", async () => {
    ...
    assert.strictEqual(
      status.canAcceptBets,
      true,
      "Should accept bets initially"
    );
...
describe("Server Actions - Single Game Flow", () => {
  ...
  it("should complete full game: bet → dive → win → surface", async () => {
    ...
    assert.ok(
      typeof diveResult.survived === "boolean",
      "Should have survival status"
    );
    ...
    if (diveResult.survived) {
      ...
      assert.strictEqual(
        afterSurface.balance,
        expectedBalance,
        "Balance should reflect win"
      );
      assert.strictEqual(
        afterSurface.gamesPlayed,
        1,
        "Game count should increment"
      );
      ...
      assert.strictEqual(
        afterDrown.balance,
        1000 - initialBet,
        "Balance should stay at bet amount"
      );
...
describe("Server Actions - Multi-Turn Games", () => {
  ...
  it("should handle 10 consecutive successful dives", async () => {
    ...
    for (let dive = 1; dive <= 10; dive++) {
      ...
      console.log(
        `  Dive ${dive}: $${currentTreasure} (${(result.survivalProbability * 100).toFixed(1)}% chance)`
      );
    }
    ...
    if (allSurvived) {
      ...
      assert.strictEqual(surfaceResult.success, true, "Surface should succeed");
      assert.ok(
        currentTreasure > initialBet * 10,
        "Should have substantial treasure"
      );
  ...
  it("should handle 5 games in sequence with mixed outcomes", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        if (!startResult.success) {
          console.log(
            `  Game ${i + 1}: Failed to start - ${startResult.error}`
          );
          continue;
        }
    ...
    const finalWallet = await getWalletInfo(userId);
    assert.ok(
      finalWallet.gamesPlayed >= 3,
      `Should have at least 3 games (got ${finalWallet.gamesPlayed})`
    );
  ...
  it("should handle deep diving with multiple successes", async () => {
    ...
    for (let dive = 1; dive <= 20; dive++) {
      ...
      if (dive % 5 === 0) {
        console.log(
          `  Dive ${dive}: $${currentTreasure} (${(result.survivalProbability * 100).toFixed(1)}%)`
        );
      }
    }
...
describe("Server Actions - Edge Cases", () => {
  ...
  it("should handle rapid consecutive games", async () => {
    ...
    const wallet = await getWalletInfo(userId);
    assert.ok(
      wallet.gamesPlayed >= 3,
      `Should have at least 3 games (got ${wallet.gamesPlayed})`
    );
    assert.ok(
      gamesCompleted >= 3,
      `Should complete at least 3 games (got ${gamesCompleted})`
    );
  ...
  it("should verify house reserves update correctly", async () => {
    ...
    if (startResult.success) {
      ...
      assert.ok(
        houseAfter.reservedFunds >= startReserves,
        "Reserves should increase or stay same"
      );
      ...
      assert.ok(
        houseEnd.reservedFunds <= houseAfter.reservedFunds,
...
describe("Server Actions - Stress Testing", () => {
  ...
  it("should handle 100 concurrent users", async () => {
    ...
    assert.ok(
      successful >= 10,
      `At least 10% should succeed (got ${successful}/100)`
    );
