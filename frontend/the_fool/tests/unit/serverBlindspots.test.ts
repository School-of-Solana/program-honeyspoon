...
describe('Session Creation Security', () => {
  ...
  it('should reject bet when user has insufficient balance', async () => {
    ...
    assert.ok(result.error?.toLowerCase().includes('maximum'), 'Error should mention maximum bet');
  ...
  it('should reject bet below minimum', async () => {
    ...
    assert.ok(result.error?.toLowerCase().includes('minimum'), 'Error should mention minimum');
  });
  ...
  it('should reject bet above maximum', async () => {
    ...
    assert.ok(result.error?.toLowerCase().includes('maximum'), 'Error should mention maximum');
  });
...
describe('Session Hijacking Prevention', () => {
  ...
  it('should reject dive from wrong user', async () => {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('does not belong'), 'Error should mention wrong user');
    }
  ...
  it('should reject dive on non-existent session', async () => {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('Invalid or inactive'), 'Error should mention invalid session');
    }
  });
  ...
  it('should reject dive on inactive session', async () => {
    ...
    if (!diveResult.survived) {
      ...
      try {
        ...
        assert.ok((error as Error).message.includes('Invalid or inactive'), 'Should mention inactive');
      }
      
      console.log('✓ Inactive session diving blocked');
    } else {
      console.log('⚠ Survived with roll=0 (unlikely), skipping inactive session test');
    }
  });
...
describe('Double Cash-Out Prevention', () => {
  ...
  it('should reject double cash-out', async () => {
    ...
    const cashOut1 = await surfaceWithTreasure(diveResult.totalTreasure, sessionId, userId);
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('Invalid or inactive'), 'Should mention invalid session');
    }
    
    // Verify balance didn't change on second attempt
    const balanceAfterSecond = (await getWalletInfo(userId)).balance;
    assert.strictEqual(balanceAfterSecond, balanceAfterFirst, 'Balance should not change on second cash-out');
  ...
  it('should reject cash-out after death', async () => {
    ...
    if (!diveResult.survived) {
      ...
      try {
        ...
        assert.ok((error as Error).message.includes('Invalid or inactive'), 'Should mention inactive session');
      }
...
describe('Cash-Out Tampering Prevention', () => {
  ...
  it('should reject inflated cash-out amount', async () => {
    ...
    console.log(`✓ Inflated cash-out blocked (${actualTreasure} → ${inflatedAmount})`);
  });
  ...
  it('should reject deflated cash-out amount', async () => {
    ...
    console.log(`✓ Deflated cash-out blocked (${actualTreasure} → ${deflatedAmount})`);
  });
  ...
  it('should reject zero cash-out', async () => {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('No value'), 'Should mention no value');
    }
  });
...
describe('Round Execution Edge Cases', () => {
  ...
  it('should reject round 0', async () => {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('Invalid round'), 'Should mention invalid round');
    }
  });
  ...
  it('should reject round beyond maxRounds', async () => {
    ...
    try {
      ...
      assert.ok(
        (error as Error).message.includes('Invalid round') || (error as Error).message.includes('1-50'),
  ...
  it('should reject negative treasure value', async () => {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes('Invalid current value'), 'Should mention invalid value');
    }
  });
...
describe('Concurrent Operations', () => {
  ...
  it('should handle concurrent dive attempts safely', async () => {
    ...
    console.log(`  Concurrent dives: ${fulfilled} succeeded, ${rejected} failed`);
