...
test.describe('Comprehensive Animation & Game Tests', () => {
  ...
  test('1. System Initialization - Complete Stack', async ({ page }) => {
    ...
    // Canvas layer checks
    const hasCanvasInit = canvasLogs.some(log => log.includes('Kaplay initialized'));
    const hasSceneCreated = canvasLogs.some(log => log.includes('Ocean scene created'));
  ...
  test('3. Game Start Transition - UI Flow', async ({ page }) => {
    ...
    const hasStartLog = gameLogs.some(log => log.includes('Starting new game'));
    expect(hasStartLog).toBeTruthy();
    console.log('  Game start logged: ✅');
    
    // Betting card should fade out
    await page.waitForTimeout(1000);
    const bettingCardGone = !(await page.locator('text=ABYSS FORTUNE').isVisible());
  ...
  test('4. Diving Animation - Full Sequence with Timing', async ({ page }) => {
    ...
    const hasTrigger = canvasLogs.some(log => log.includes('Triggering diving animation'));
    expect(hasTrigger).toBeTruthy();
    console.log('  Animation triggered: ✅');
    
    // Check for state logging during animation
    await page.waitForTimeout(1000);
    const hasStateLog = canvasLogs.some(log => log.includes('[CANVAS STATE]'));
    console.log('  State logging active:', hasStateLog ? '✅' : '⚠️  (may not trigger in 1s)');
    ...
    const hasCompletion = canvasLogs.some(log => log.includes('Diving animation complete'));
    ...
    const depthText = await page.locator('div').filter({ hasText: /^\d+m$/ }).first().textContent({ timeout: 2000 });
    console.log('  Depth after dive:', depthText);
    
    // Check treasure updated (or game over)
    const isDrowned = await page.locator('text=DROWNED').isVisible().catch(() => false);
    if (isDrowned) {
      console.log('  Result: 💀 DROWNED');
      const hasDeathAnim = canvasLogs.some(log => log.includes('death animation'));
      console.log('  Death animation triggered:', hasDeathAnim ? '✅' : '❌');
    } else {
      console.log('  Result: ✅ SURVIVED');
      const hasTreasureAnim = canvasLogs.some(log => log.includes('treasure animation'));
      console.log('  Treasure animation triggered:', hasTreasureAnim ? '✅' : '❌');
    }
  });
  ...
  test('5. Multiple Dives - State Consistency', async ({ page }) => {
    ...
    for (let i = 1; i <= maxDives; i++) {
      ...
      const depthBefore = await page.locator('div').filter({ hasText: /^\d+m$/ }).first().textContent().catch(() => '0m');
      const treasureBefore = await page.locator('div').filter({ hasText: /^\$\d+$/ }).first().textContent().catch(() => '$0');
      ...
      if (isDrowned) {
        ...
        // Verify reset
        await page.waitForTimeout(4000);
        const resetCard = await page.locator('text=ABYSS FORTUNE').isVisible().catch(() => false);
        expect(resetCard).toBeTruthy();
        console.log('  Game reset to betting screen: ✅');
        break;
      } else {
        console.log('  ✅ SURVIVED');
        const depthAfter = await page.locator('div').filter({ hasText: /^\d+m$/ }).first().textContent().catch(() => '0m');
        const treasureAfter = await page.locator('div').filter({ hasText: /^\$\d+$/ }).first().textContent().catch(() => '$0');
  ...
  test('6. Surface Success Flow', async ({ page }) => {
    ...
    // Get treasure value
    const treasureText = await page.locator('div').filter({ hasText: /^\$\d+$/ }).first().textContent();
    ...
    await page.waitForTimeout(500);
    const hasSurfaceLog = gameLogs.some(log => log.includes('Surfacing with'));
  ...
  test('7. Rapid Click Protection', async ({ page }) => {
    ...
    const diveInitLogs = gameLogs.filter(log => log.includes('Dive initiated'));
