...
test.describe("Abyss Fortune Game Flow", () => {
  ...
  test("Step 3: Start Game - Betting Card Disappears, HUD Appears", async ({
    ...
    const hasStartLog = consoleLogs.some((log) =>
      log.includes("[GAME] Starting new game with bet: $100")
    );
    const hasHUDLog = consoleLogs.some((log) =>
      log.includes("[GAME] Game started, HUD visible")
    );
  ...
  test("Step 4: First Dive - Animation and Result", async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const hasDiveInitLog = consoleLogs.some((log) =>
    ...
    const hasServerLog = consoleLogs.some((log) =>
    ...
    if (isDrowned) {
      ...
      const hasDrownedLog = consoleLogs.some((log) =>
  ...
  test("Step 6: Surface Successfully", async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const hasSurfaceLog = consoleLogs.some((log) =>
