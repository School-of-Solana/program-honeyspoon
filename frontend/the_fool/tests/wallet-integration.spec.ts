...
test.describe('Wallet Integration', () => {
  ...
  test('01 - should display initial wallet balance of $1000', async ({ page }) => {
    // Check wallet balance is displayed
    const balanceText = await page.locator('text=/Your Balance/').locator('..').locator('span').last().textContent();
    expect(balanceText).toContain('$1000');
    
    await page.screenshot({ path: 'tests/screenshots/wallet-01-initial-balance.png', fullPage: true });
  });

  test('02 - should prevent betting more than wallet balance', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-02-insufficient-balance.png', fullPage: true });
  });

  test('03 - should deduct bet from balance when starting game', async ({ page }) => {
    // Place $100 bet
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-03-bet-placed.png', fullPage: true });
  });
  ...
  test('04 - should update balance after losing game', async ({ page }) => {
    ...
    while (attempts < 20) {
      try {
        ...
        if (await diveButton.isVisible() && !await diveButton.isDisabled()) {
    ...
    const newBalance = await page.locator('text=/Your Balance/').locator('..').locator('span').last().textContent();
    expect(newBalance).toContain('$900');
    
    await page.screenshot({ path: 'tests/screenshots/wallet-04-after-loss.png', fullPage: true });
  });

  test('05 - should update balance after winning (surfacing)', async ({ page }) => {
    // Place $50 bet
    ...
    if (await surfaceButton.isVisible() && !await surfaceButton.isDisabled()) {
      ...
      await page.screenshot({ path: 'tests/screenshots/wallet-05-after-win.png', fullPage: true });
    } else {
  ...
  test('06 - should enforce house betting limits', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-06-house-limits.png', fullPage: true });
  });
  ...
  test('07 - should prevent multiple simultaneous games', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-07-single-game.png', fullPage: true });
  });

  test('08 - should show correct max bet based on balance', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-08-max-bet.png', fullPage: true });
  });
  ...
  test('09 - should validate bet on every change', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-09-validation.png', fullPage: true });
  });
  ...
  test('10 - should handle edge case: exact balance bet', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-10-exact-balance.png', fullPage: true });
  });

  test('11 - should persist balance across page navigation', async ({ page }) => {
    // Reload page
    await page.reload();
    await page.waitForTimeout(1000);
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-11-reload.png', fullPage: true });
  });
  ...
  test('12 - should show balance in betting card UI', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-12-ui-display.png', fullPage: true });
  });
  ...
  test('13 - should handle rapid bet changes gracefully', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-13-rapid-changes.png', fullPage: true });
  });
  ...
  test('14 - should show limit warning when appropriate', async ({ page }) => {
    ...
    // Just capture state
    await page.screenshot({ path: 'tests/screenshots/wallet-14-limit-warning.png', fullPage: true });
  });
  ...
  test('15 - should handle minimum bet correctly', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/wallet-15-min-bet.png', fullPage: true });
  });
});
