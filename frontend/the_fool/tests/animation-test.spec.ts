...
test.describe('Animation System Tests', () => {
  ...
  test('Canvas Initialization Test', async ({ page }) => {
    ...
    const hasInitLog = canvasLogs.some(log => log.includes('🎬 OceanScene useEffect triggered'));
    const hasKaplayInit = canvasLogs.some(log => log.includes('✅ Kaplay initialized'));
    ...
    const hasHeartbeat = canvasLogs.some(log => log.includes('💓 Update loop heartbeat'));
  ...
  test('Diving Animation Triggers', async ({ page }) => {
    ...
    const hasPropUpdate = canvasLogs.some(log => 
      log.includes('[CANVAS PROPS UPDATE]') && log.includes('isDiving: true')
    );
    console.log('  Props updated (isDiving: true):', hasPropUpdate ? '✅' : '❌');
  ...
  test('State Machine Logging', async ({ page }) => {
    ...
    const stateLogs = canvasLogs.filter(log => log.includes('[CANVAS STATE]'));
