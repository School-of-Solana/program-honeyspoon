...
echo -e "${BLUE}🔄 Syncing IDL from Anchor to Frontend...${NC}\n"
...
echo "⚓ Anchor project: $ANCHOR_DIR"
echo "🎨 Frontend project: $FRONTEND_DIR"
echo ""

# Step 1: Check if anchor IDL exists
if [ ! -f "$ANCHOR_IDL" ]; then
    echo -e "${RED}❌ Error: Anchor IDL not found at $ANCHOR_IDL${NC}"
    echo -e "${YELLOW}💡 Run 'anchor build' first to generate the IDL${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Found Anchor IDL${NC}"
...
if [ -f "$FRONTEND_IDL_JSON" ]; then
    if cmp -s "$ANCHOR_IDL" "$FRONTEND_IDL_JSON"; then
        echo -e "${GREEN}✅ IDL already in sync, no changes needed${NC}"
        exit 0
    else
        echo -e "${YELLOW}⚠️  IDL out of sync, updating...${NC}"
    fi
else
    echo -e "${YELLOW}⚠️  Frontend IDL doesn't exist, creating...${NC}"
fi

# Step 4: Copy JSON IDL
echo -e "${BLUE}📋 Copying dive_game.json...${NC}"
cp "$ANCHOR_IDL" "$FRONTEND_IDL_JSON"
echo -e "${GREEN}✅ Copied JSON IDL${NC}"
...
npx @coral-xyz/anchor idl convert \
    ...
    echo -e "${YELLOW}⚠️  anchor idl convert not available, using fallback...${NC}"
...
export const DISCRIMINATORS = {\`;
    ...
    echo -e "${GREEN}✅ Updated instruction discriminators${NC}"
else
    echo -e "${YELLOW}⚠️  solanaHelpers.ts not found, skipping discriminator update${NC}"
fi

# Step 10: Summary
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✨ IDL Sync Complete!${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "${BLUE}📦 Files Updated:${NC}"
echo "   ✅ $FRONTEND_IDL_JSON"
echo "   ✅ $FRONTEND_IDL_TS"
echo "   ✅ $SOLANA_HELPERS (discriminators)"
echo ""
